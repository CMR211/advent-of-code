...
function solve7b(input) {
    ...
    for (let target = 0; target <= max; target++) {
        const fuel = input.map((n) => specialFuelConsumption(n, target)).reduce((a, b) => a + b)
        consumptions.push(fuel)
    }
    const minConsumptions = Math.min(...consumptions)
    const optPos = consumptions.indexOf(minConsumptions)
    console.log("Optimal Position:", optPos, "| Fuel Consumption:", minConsumptions)
}
...
function solve7a(input) {
    ...
    console.log("Optimal Position:", optimalPosition, "| Fuel Consumption:", fuelConsumption)
