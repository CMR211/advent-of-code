...
const lines = filterLinesFromInput(parseInput(data))
const diagonals = filterDiagonalsFromInput(parseInput(data))
...
function drawLine(line, diagram) {
    ...
    diagram = diagram.map((cell) => {
        if (cell[0] >= xmin && cell[0] <= xmax && cell[1] >= ymin && cell[1] <= ymax) cell[2] = cell[2] + 1
    })
}
...
function filterLinesFromInput(input) {
    return input.filter((coordinate) => {
        if (coordinate.x1 === coordinate.x2 || coordinate.y1 === coordinate.y2) return true
    })
}
